...
2. 可以对查出的单词意思进行修改；
3. 可以对字典中的单词进行删除操作；
4. 实现生词本功能，用户可自主选择将生词保存起来，形成新文件；
5. 点进修改与删除界面可以看到当前所有单词表，可通过选中行进行删改
...
- 继承自JFrame
- 用于设计主面板布局和各按钮触发功能
- 按钮：查询(调用do_button_actionPerformed方法)、添加（调用AddWin()外部类）、修改和删除（调用DelWin()外部类
...
3. DelWin
- 继承自JFrame
- 用于设计修改和删除界面并实现修改与删除功能（调用内部updatecsv和delcsv方法）

## 方法的功能说明

1. do_button_actionPerformed
- 用来响应按钮单击事件。在该方法中完成对单词的查询操作，并根据不同的情况进行提示


## 涉及知识点阐述
- 如面向对象、集合、异常处理、线程，图形用户界面，或者 I/O 流等

### 图形用户界面
- 使用import javax.swing.*;和import java.awt.*;包
- 使用举例：
1. 创建组件
- 使用JFrame框架
```java
...
```
2. 指定布局
- 本次设计中部分采用绝对布局，部分采用BorderLayout
```java
...
### 异常处理
- 设计中多次使用try...catch异常处理结构
- 使用举例：查找单词对应翻译时，如果单词不存在则提示查找失败
```java
try {
	List<String> allString = new ArrayList<>();//定义一个字符串数组
	while ((line = br.readLine()) != null) // 读取到的内容给line
...
### 工具类
- 本次设计中主要使用ArrayList
- 调用java.util.ArrayList;和java.util.List;
```java
...
- 这次设计作业基于曾经的一次实验题目展开，更多的功能实现让我将所学过的零碎的知识点系统的串起来并合理使用，对于java这门面向对象的编程语言有了更深刻的理解和更好的掌握。虽然在编写代码的过程中遇到了各种各样的困难，但最终都通过网络搜索和课件搜索解决了，还学会了课本例子和实验中未曾用到过的内容，培养了自主学习的能力，收益颇丰。